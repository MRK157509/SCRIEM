...
function getAlertKey(item) {
  ...
  return String(
    item?.id ||
      ...
      item?.event_id ||
      item?.key ||
      `${item?.title || "untitled"}|${item?.host || "nohost"}|${item?.user || "nouser"}|${
        item?.ip || "noip"
      }`
  );
}
...
function extractIocs(item) {
  ...
  const domains = new Set(
    raw.match(/\b(?!(?:https?:\/\/))(?:(?:[a-z0-9-]{1,63}\.)+(?:[a-z]{2,24}))\b/gi) || []
  );
  const hashes = new Set(
    raw.match(/\b[a-f0-9]{32}\b|\b[a-f0-9]{40}\b|\b[a-f0-9]{64}\b/gi) || []
  );
...
function maskIocs(iocs) {
  return {
    ips: (iocs.ips || []).map(maskIp),
    domains: (iocs.domains || []).map(maskDomain),
    hashes: (iocs.hashes || []).map(maskHash),
  };
}
...
export default function RightDrawer({
  ...
  // Table mapping:
  // USER: limited status change (triage only)
  // SOC_ANALYST: full
  // ADMIN: full
  const canTriage = role === "USER" || role === "SOC_ANALYST" || role === "ADMIN";
  const canEscalate = role === "SOC_ANALYST" || role === "ADMIN";
  const canClose = role === "SOC_ANALYST" || role === "ADMIN";

  // IOC values: USER masked, others full
  const iocMode = role === "USER" ? "masked" : "full";

  const alertKey = useMemo(() => getAlertKey(item), [item]);

  const LS_ACTIONS_KEY = useMemo(() => `scriem.investigation.actions.${alertKey}`, [alertKey]);
  const LS_ACTIVITY_KEY = useMemo(() => `scriem.investigation.activity.${alertKey}`, [alertKey]);
  ...
  const onEscalate = () => updateActions({ escalated: true }, ["Escalated to higher tier"]);
  ...
  const iocsFull = useMemo(() => extractIocs(item), [item]);
  const iocsForUser = useMemo(
    () => (iocMode === "masked" ? maskIocs(iocsFull) : iocsFull),
    [iocMode, iocsFull]
  );

  const handleCopyIocs = async () => {
    const ok = await copyToClipboard(buildIocCopyText(iocsForUser));
      ...
      {showMiniPanel && (
        <div className="fixed top-0 right-0 h-full z-30 border-l border-slate-800 bg-slate-950/70 backdrop-blur shadow-xl w-[120px]">
          ...
          <div className="p-3 flex flex-col gap-3">
            ...
            {/* Evidence stats are allowed for all roles */}
            <div className="border border-slate-800 rounded-lg p-2 text-center">
              <div className="text-[10px] text-slate-400">IPs</div>
              <div className="text-sm text-white font-semibold">{iocsForUser.ips.length}</div>
            </div>

            <div className="border border-slate-800 rounded-lg p-2 text-center">
              <div className="text-[10px] text-slate-400">Domains</div>
              <div className="text-sm text-white font-semibold">{iocsForUser.domains.length}</div>
            </div>

            <div className="border border-slate-800 rounded-lg p-2 text-center">
              <div className="text-[10px] text-slate-400">Hashes</div>
              <div className="text-sm text-white font-semibold">{iocsForUser.hashes.length}</div>
            </div>
            ...
            <button
              ...
              title={iocMode === "masked" ? "Copy masked IOCs" : "Copy extracted IOCs"}
            >
              {copyState === "copied"
                ? "Copied!"
                : copyState === "failed"
            ...
            <div className="h-full flex flex-col">
              <div className="p-4 border-b border-slate-800">
                ...
                {!isCollapsed && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {/* Limited status: USER can triage only */}
                    {canTriage ? (
                      <button
                        onClick={onTriage}
                    ...
                    {/* IOC copy (masked for USER, full for others) */}
                    <button
                      onClick={handleCopyIocs}
              ...
              {!isCollapsed ? (
                ...
                <div className="flex-1 p-3 flex flex-col gap-3 items-center justify-start">
                  {/* Collapsed evidence stats are allowed */}
